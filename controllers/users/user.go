...
func (this *AvatarUserController) Post() {
	...
	dst, err := utils.LoadImage("." + filepath)
	if err != nil {
		this.Data["json"] = map[string]interface{}{"code": 0, "message": "头像上传失败！" + err.Error()}
		this.ServeJSON()
		return
	}
	croppedImg, err := cutter.Crop(dst, cutter.Config{
		Width:  dataWidth,
		Height: dataHeight,
		Anchor: image.Point{X: dataX, Y: dataY},
...
// AjaxStatusUserController 用户状态更改异步操作
